...
tool_node = ToolNode(tools=tools, messages_key="uni_message")

# nodes
def uni_agent(state: PathFinderState) -> dict:
    messages        = state["uni_message"]
    stage_reasoning = get_stage_reasoning(state)
    uni             = state.get("university")
    ...
    is_current_stage = (state.get("stage", {}).get("current_stage") == "university")

    response = agent_llm.invoke(
        [SystemMessage(UNI_DRILL_PROMPT.format(
            is_current_stage=is_current_stage,
            stage_reasoning=stage_reasoning.uni,
            uni=uni or "",
    ...
    if hasattr(response, "tool_calls") and response.tool_calls:
        return {"uni_message": response} # tools_condition needs this to route
        
    # Branch 2: LLM wrote the final PROBE reasoning
    updated = stage_reasoning.model_copy(update={"university": response.content})
    return {"stage_reasoning": updated.model_dump(), "uni_message": response}
...
def confident_node(state: PathFinderState) -> dict:
    ...
    return {"university": response.university.model_dump()}
...
builder.add_conditional_edges(
    "uni_agent",
    lambda state: tools_condition(state, messages_key="uni_message"),
    ["tools", END]
)
