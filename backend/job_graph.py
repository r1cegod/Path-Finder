...
from backend.data.state import PathFinderState, JobProfile, JobResearch, StageReasoning
from backend.data.prompts.job import (
    JOB_CONFIDENT_PROMPT,
    JOB_RESEARCH_PLAN_PROMPT,
    JOB_SYNTHESIS_PROMPT,
)
...
class JobResearchPlan(BaseModel):
    model_config = ConfigDict(extra="forbid")
    need_research: bool
    query_focus: str
...
DOMAIN_BUCKETS: dict[str, tuple[str, ...]] = {
    "job_salary": JOB_SALARY_DOMAINS,
    "job_role_reality": JOB_ROLE_REALITY_DOMAINS,
    "job_market": JOB_MARKET_DOMAINS,
    "none": (),
}
...
def get_job_research(state: PathFinderState) -> JobResearch:
    raw = state.get("job_research")
    if raw is None:
        return JobResearch()
    if isinstance(raw, dict):
        return JobResearch(**raw)
    return raw
...
def _infer_probe_field(job: object) -> str:
    if job is None:
        return "day_to_day"
...
def _default_probe_instruction(field_name: str) -> str:
    defaults = {
        "day_to_day": "force the student to choose between the role fantasy and the recurring grind it actually requires.",
        "autonomy_level": "force a trade-off between freedom and the management, client, or team dependence the job imposes.",
        "company_stage": "force a choice between the claimed company environment and the operational costs it brings.",
        "role_category": "force the student to defend the role against the strongest prior or market contradiction.",
    }
...
def _compose_probe_line(response: JobAnalysis, is_current_stage: bool, job: object) -> str:
    ...
    if not field_name or field_name.upper() == "NONE":
        field_name = _infer_probe_field(job)
...
def _run_web_research(packet: JobResearch) -> JobResearch:
...
def job_research_planner(state: PathFinderState) -> dict:
    ...
    job = state.get(PROFILE_KEY)
    job_research = get_job_research(state)
    ...
    response = planner_llm.invoke(
        [SystemMessage(JOB_RESEARCH_PLAN_PROMPT.format(
    ...
    return {"job_research": packet.model_dump()}


def job_researcher(state: PathFinderState) -> dict:
    packet = get_job_research(state)
    if not packet.need_research or not packet.search_query:
        return {"job_research": packet.model_copy(update={"research_complete": False}).model_dump()}

    researched = _run_web_research(packet)
    return {"job_research": researched.model_dump()}


def job_synthesizer(state: PathFinderState) -> dict:
    messages = state[QUEUE_KEY]
    stage_reasoning = get_stage_reasoning(state)
    job = state.get(PROFILE_KEY)
    job_research = get_job_research(state)
    ...
    response = analysis_llm.invoke(
        [SystemMessage(JOB_SYNTHESIS_PROMPT.format(
    ...
    summary_body = _strip_legacy_probe_suffix(response.job_summary)
    probe_line = _compose_probe_line(response, is_current_stage, job)
...
builder.add_conditional_edges(
    ...
    route_after_planner,
    ["job_researcher", "job_synthesizer"],
)
builder.add_edge("job_researcher", "job_synthesizer")
builder.add_edge("job_synthesizer", END)
